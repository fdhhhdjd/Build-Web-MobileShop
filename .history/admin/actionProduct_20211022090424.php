<?php
	...
	if(isset($_POST['update'])){
		...
		$image=$_POST['oldimage'];
		$date=$_POST['date'];
		$query="UPDATE product SET item_id=?,item_brand=?,item_name=?,item_price=?,item_image=?,item_register=? WHERE item_id=?";
		$stmt=$conn->prepare($query);
		$stmt->bind_param("ssssssi,$id,$brand,$name,$price,$image,$date);
		$stmt->execute();
		$_SESSION['response']="Updated Successfully!";
		$_SESSION['res_type']="primary";
		header('location:product.php');
